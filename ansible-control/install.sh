...
###############################
## Azure Ansible Credentials ##
###############################
tput setaf 1; echo 'Creating Dynamic Inventory File...' ; tput sgr0
tput setaf 1; echo '----------------------------------' ; tput sgr0
...
read ANSIBLE_SP_ID ANSIBLE_SP_PASSWORD AZURE_AD_TENANT <<< $(az ad sp create-for-rbac -n ${AZURE_GROUP}-${DATE}-Ansible --query [appId,password,tenant] -o tsv)

cat <<EOF > ${DATE}-Credentials
[default]
subscription_id=${AZURE_SUBSCRIPTION}
client_id=${ANSIBLE_SP_ID}
secret=${ANSIBLE_SP_PASSWORD}
tenant=${AZURE_AD_TENANT}
EOF

az storage blob upload \
  --container-name scripts \
  --file ./${DATE}-Credentials \
  --name credentials \
  --account-name ${AZURE_STORAGE_ACCOUNT} \
  --account-key ${AZURE_STORAGE_KEY}
